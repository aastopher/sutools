...
def test_timeout(mock_os, mock_atexit_register):
    ...
    for filetimeout, expected_removed_files in [('1y', 1), ('1o', 1), ('30d', 1), ('5h', 2), ('30m', 3), ('1z', 0)]:
        if filetimeout[-1] not in time_units:
            ...
            with patch('builtins.open', side_effect=mock_files), patch('warnings.warn') as mock_warn:
                log_obj = log_handler.Logger(
                    name='test_name',
                    loggers=['log'],
                    loglvl=logging.INFO,
                    ...
                    shandler=None,
                    stream=False)
                mock_warn.assert_called_once_with(f"Invalid time unit: {filetimeout[-1]}", Warning)
        else:
